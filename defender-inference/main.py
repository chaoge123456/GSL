...
from network import client_resnet, server_tasker, server_decoder
from train import CleanGroup, MaliciousGroup, predict, reconstruct
...
def getImagesDS(X, n):
    image_list = []
    for i in range(n):
        image_list.append(X[i+100][0].numpy()[None,])
...
def global_predict(ep, args, logger, in_dim, global_client, global_server, decoder, cleantest):
    ...
    avg_rec_loss = reconstruct(cleantest, clientmodel, decoder, args.device)
    logger.add_scalar("Global/client", accclean, ep)
    logger.add_scalar("Global/attack", avg_rec_loss, ep)
    print("Global Client Accuracy:{:.4f}\tGlobal Client Attack:{:.4f}".format(accclean, avg_rec_loss))
...
def args_parser(log_time):
    ...
    parser.add_argument("--mode", type=str, default='clean', help='clean, attack, defense')
    parser.add_argument("--step", type=str, default='fsha', help='fsha, plus')
    parser.add_argument("--aggregation", type=str, default='krum', help='average, krum, bulyan')
    parser.add_argument('--select', type=int, default=4, help="number of select groups for update")
    ...
    parser.add_argument('--malicious', type=int, default=1, help="number of malicious server")
    ...
    parser.add_argument('--lr_client', type=float, default=0.0001, help='learning rate client')
    parser.add_argument('--lr_server', type=float, default=0.0001, help="learning rate server")
    parser.add_argument('--lr_D', type=float, default=0.0002, help="learning rate D")
...
if __name__ == "__main__":
    ...
    logger = SummaryWriter(Path('./log/') / log)
    ...
    showimage = torch.from_numpy(getImagesDS(testdata, 16)).cuda()
    ...
    if args.mode == "clean":  # 正常训练
        ...
        for ep in range(args.epochs):
            print("\n******************************************Epoch:{}************************************************".format(ep))
            group.train(logger, cleantest, showimage)
        ...
        for ep in range(args.epochs):
            ...
            for g in group_list:
                if g in malicious_group_id:
                    ...
                    groups[g].train(logger, cleantest, showimage)
                    decoder = copy.deepcopy(groups[g].server.server_decoder)
                else:
            ...
            global_predict(ep, args, logger, in_dim, global_client, global_server, decoder, cleantest)
