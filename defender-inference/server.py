...
from network import server_pilot, server_tasker, discriminator, server_dis, server_decoder
...
class MaliciousServer:
    ...
    def __init__(self, args, in_dim):
        ...
        self.server_decoder = server_decoder(args.level, in_dim)

        self.server_pilot.apply(init_weights)
        self.server_model.apply(init_weights)
        self.discriminator.apply(init_weights)
        self.server_decoder.apply(init_weights)
        self.optim = torch.optim.Adam([{'params': self.server_pilot.parameters()}, {'params': self.server_model.parameters()}, {'params': self.server_decoder.parameters()}], lr=args.lr_server)
    ...
    def serverdata(self):
        ...
        serverloader = torch.utils.data.DataLoader(server_data, batch_size=self.args.batch_size, shuffle=False)
        return serverloader
