...
class Discriminator_server(nn.Module):

    def __init__(self, level, bn=False):
        super(Discriminator_server, self).__init__()
        self.level = level
        self.channel = set_input_shape_server(level)
        self.size = set_input_shape_size(level)
        self.label_embedding = nn.Embedding(10, self.size*self.size)
        self.conv11 = nn.Conv2d(self.channel, 128, 3, 2, 1)
        self.conv12 = nn.Conv2d(128, 256, 3, 2, 1)
        self.conv23 = nn.Conv2d(self.channel, 256, 3, 2, 1)
        self.conv4 = nn.Conv2d(self.channel, 256, 3, 1, 1)
        self.conv5 = nn.Conv2d(256, 256, 3, 2, 1)
        self.linear = nn.Linear(1024, 1)
        self.block1 = ResBlock(256, 256, bn=bn)
        self.block2 = ResBlock(256, 256, bn=bn)
        self.block3 = ResBlock(256, 256, bn=bn)
        self.block4 = ResBlock(256, 256, bn=bn)
        self.block5 = ResBlock(256, 256, bn=bn)
        self.block6 = ResBlock(256, 256, bn=bn)

    def forward(self, x, label):
        embedding = self.label_embedding(label).reshape(-1, 1, self.size, self.size)
        x = x + embedding
